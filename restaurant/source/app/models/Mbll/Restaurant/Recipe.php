...
/**
 * Mobile Restaurant recipe logic
 *
...
class Mbll_Restaurant_Recipe extends Mbll_Abstract
{

    /**
     * get nb recipe by genre
     *
     * @param string $genre
     * @return array
     */
    public function getNbRecipeByGenre($genre) {
        $recipeList = array();
        
        $nbRecipList = Mbll_Restaurant_Cache::getRecipe();
        
        foreach ($nbRecipList as $recipe) {
        	if ($genre == $recipe['genre']) {
        	    $recipeList[] = $recipe;
        	}
        }
     ...
     * @return array
     */
    public function getUserRecipeByGenre($genre, $uid) {
        $recipeList = array();
        
        $mdalResRecipe = Mdal_Restaurant_Recipe::getDefaultInstance();
        $userRecipList = $mdalResRecipe->getUserRecipeList($uid);
        
        foreach ($userRecipList as $recipe) {
            if ($genre == $recipe['genre']) {
                if (!empty($recipe['food'])) {
                    $recipeList =  Zend_Json::decode($recipe['food']);
                }
            }
        }
     ...
     * @return string
     */
    public function getNbGenreName($id) {
        $name = "";
        
        $nbGenreList = Mbll_Restaurant_Cache::getNbGenreList();
        
        foreach ($nbGenreList as $genre) {
            if ($id == $genre['genre']) {
                $name = $genre['name'];
            }
        }
    ...
    public function updateUserFoodCookTimes($uid, $food)
    {
        $mdalRecipe = Mdal_Restaurant_Recipe::getDefaultInstance();
        $genre = $mdalRecipe->getFoodGenre($food);
        
        if (empty($genre)) {
            return;
        }
        
        $genreFood = $mdalRecipe->getUserGenreFood($uid, $genre);
        $genreFood = Zend_Json::decode($genreFood['food']);
        
        if (empty($genreFood) || empty($genreFood[$food])) {
            return;
        }
        
        $genreFood[$food]['times'] = $genreFood[$food]['times'] + 1;
        $genreFood = array('food' => Zend_Json::encode($genreFood));
        $mdalRecipe->update($uid, $genre, $genreFood);
    }
}